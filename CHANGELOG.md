...
* **`Window > UnityFlow`** — a runner window. It picks a flow, starts it through the same
  `flow.run` / `flow.start` commands the CLI uses, and reads its state from the run's own
  `progress.ndjson`, so a run started from anywhere is rendered the same and one that crossed a
  domain reload is rebuilt by replaying the file.
  What it shows is the point. The picker lists a flow by the `name:` it declares, bold, over its
  path elided from the LEFT so the distinguishing tail survives, grouped into collapsible folders.
  A step row is `[state] verb argument … duration`, and the ARGUMENT is whatever identifies that
  step among the others with the same verb — the selector for `tapOn`, the text for `inputText`,
  the assembled member and comparison for `assert`, the key for `press` — cut in the middle with an
  ellipsis and carried in full in the tooltip, because six rows reading `assert` say nothing.
  `before` / `steps` / `after` are labelled, `runFlow`'s spliced steps are indented under their
  source file named once, durations sit in a fixed right-aligned column, and a failure opens inline
  under the step that produced it with the UI snapshot in a monospace pane and the screenshot both
  as a path and as a thumbnail. The whole palette lives in `FlowRunnerWindow.uss`, declared once per
  editor skin.
  The `progress.ndjson` fold is `FlowRunProgress`, which knows nothing about UI, and the step
  caption is `FlowStepCaption`, a pure function of a parsed step; both are covered by tests.
...
* **`Samples~/CourierDemo`** — a small complete GAME, built from Unity primitives so it needs no art
  and no other package, plus six flows that drive it and are numbered so the list reads as a tour:
  a menu form whose Play button is dead until it is filled, the same form driven with no pointer at
  all, a round played with held keys and asserted on component fields, a drag that reorders two
  inventory slots, a pause screen checked with `stableFor`, and one flow that FAILS on purpose so
  the failure report has something to be demonstrated on. It exercises both halves of the tool that
  a flows-only sample cannot: real form controls and real gameplay. The scene is generated by an
  editor menu item rather than hand-written, so every position, colour and wiring decision is
  reviewable as code.
